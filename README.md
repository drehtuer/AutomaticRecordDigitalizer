...
A DIY machine that digitises a vinyl record collection unattended: it takes a record from a magazine, puts it on a Technics-style turntable, cleans it, cues the stylus, records the side, flips the record, records the other side, and puts it back — about 25 records per eight-hour run, with a photo of both labels for the tags.
...
Concept design, finished and reviewed. Nothing has been bought or printed yet. The next steps are the parametric CAD (OpenSCAD/CadQuery), the Klipper configuration, and the Python orchestrator.
