...
The gripper is a single 40 mm bellows vacuum cup on the label area, fed by a 12 V diaphragm pump through a solenoid release valve, with a pressure sensor on the line that confirms a seal before any move and aborts a move if the seal is lost. The label area is the one thing every record has in common, so the cup handles 12", 10" and 7" records and shaped discs alike; 7" singles with the large jukebox hole are out of scope. What the cup can hold and what the carousel can present are not the same set: the magazine locates a record by its round edge, so a shaped picture disc, or one whose outline is not centred on its hole, is placed on the platter by hand and the machine does the rest (see the operating cycle).
...
The magazine is a horizontal disc with 24 radial slots at 15° pitch. Records stand vertically in the slots like spokes, each in a 5 mm slot on top of a printed comb. The floor of that slot is a shallow V, sloping 5° each way from an apex 26 cm from the axis, so a record of any diameter rolls to the apex and stands with its centre on the same vertical line: 12", 10" and 7" records all sit at the same radius, and only the height of the centre differs, by the record's radius. The pick is therefore the same X and Y for every size, and the machine takes the height from the size in the batch manifest. At that radius the gap between neighbouring spokes is about 6.5 cm face to face, enough for the 5 cm gripper assembly to descend beside a record. A 10" is picked 25 mm lower between the same neighbours, through the same gap. A 7" is picked 62 mm lower, and there the gap runs out: the wrist hub, the outrigger and the foot of the column sit 22 cm above the cup and are wider than the gap at the radius where they descend, so they have to stay above the top edge of the neighbouring spoke on the front side, the side the wrist hangs on. Beside a 12" neighbour a 7" pick puts the hub 18 mm and the column's foot 7 mm below that edge. So the slot on a 7"'s front side holds a 7", a 10" or nothing, a rule the manifest editor enforces; a run of 7"s costs one empty slot at its front end. There is no rim: the V does the locating, and a wall tall enough to touch every size at its own centre would stand in the way of a record being carried past at travel height.

The disc is 18 mm plywood cut roughly round and is nothing but a floor. It rolls on ten identical printed brackets, each holding a 608 bearing, arranged on a 72 cm circle, and is located by a 6005 bearing on a stub shaft at the centre. A printed GT2 tooth ring in eight dovetailed segments on the disc's underside is driven by a NEMA17 with a 20-tooth pulley, about 34:1 at the ring and about 170:1 with the motor's 5:1 gearbox, and an optical sensor reads a home mark; one slot index is therefore repeatable to a fraction of a millimetre at the rim. The 24 slot combs plug into a printed centre hub, which carries the angular positions, and are screwed to the disc at their outer ends, so the wood never has to be cut precisely.
