...
**Controller.** Two options. The bought one is a BIGTREETECH Octopus V1.1 with five TMC2209 drivers (X, Y, Z, wrist, carousel index) carrying all I/O. The cheaper one, if the donor printer brings a Klipper-capable 32-bit board, uses that board's four drivers for X, Y, Z and the wrist, and the NXP MIMXRT1010-EVK already on hand as a second Klipper MCU for everything else: the carousel stepper through one TMC2209 stick on a small breakout, the cue-lever servo, the LED bar's 50 Hz PWM, the pump and valve through MOSFETs, and the vacuum, home and end-stop sensors. Klipper's i.MX RT port (the Teensy 4 family) lists the RT1011; confirm it in the current `menuconfig` before relying on it. Klipper runs multi-MCU setups natively. Its servo header drives the cue-lever servo; its fan/heater MOSFET outputs drive the vacuum pump, the release valve and the LED bar (the LED output must be a proper PWM pin so the strobe can run at 50.000 Hz); its end-stop inputs take the axis end-stops, the wrist Hall sensor, the carousel home sensor and the vacuum switch. A 24 V 150 W supply feeds the board, and the board's 5 V rail or a separate buck converter feeds the servos.
...
G-code macros implement the primitives the orchestrator composes: `HOME_ALL`, `GRIP` (pump on, wait for the vacuum switch, else error), `RELEASE`, `CUE_UP`, `CUE_DOWN`, `CAROUSEL_INDEX`, `STROBE_ON`, `LIGHT_ON`, `LIGHT_OFF`, and `EMERGENCY_LIFT`, which is `CUE_UP` followed by a motion halt, bound so it can be fired without waiting for the queue; the orchestrator opens the start/stop relay at the same time.
