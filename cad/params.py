...
CAR_REC_R = 260.0                           # radius of every record's centre, whatever its size: the apex of the comb's V floor
CAR_COMB_L, CAR_COMB_H, CAR_COMB_T = 300.0, 30.0, 12.0   # comb height at the apex; it rises with the V floor towards both ends
CAR_COMB_R = 240.0                          # comb centre radius, so the comb spans 90..390 from the axis
CAR_SLOT_W, CAR_SLOT_D = 5.0, 12.0          # the locating slot on top of the comb, at the apex
CAR_V_SLOPE = radians(5.0)                  # the slot floor rises from the apex towards both ends: a record of any diameter rolls to CAR_REC_R
CAR_SLOT_Z = CAR_DISC_Z + CAR_DISC_T + CAR_COMB_H - CAR_SLOT_D   # the apex of the floor; a record's centre stands its radius above it
